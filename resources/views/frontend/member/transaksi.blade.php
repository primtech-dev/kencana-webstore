        ...
        <div class="lg:grid lg:grid-cols-12 lg:gap-8 mt-6">
            ...
            <div class="lg:col-span-9">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-xl border border-gray-100">
                    <h2 class="text-2xl font-bold text-gray-800 mb-6 pb-3 border-b">Semua Pesanan</h2>
                    
                    {{-- Filter Status Transaksi --}}
                    <div class="overflow-x-auto mb-6 pb-2 whitespace-nowrap scrollbar-hide">
                        <div class="inline-flex space-x-3 text-sm p-1">
                            
                            @foreach ($statusFilters as $key => $filter)
                                @php
                                    // Tentukan class aktif
                                    $baseClass = 'py-1.5 px-4 rounded-full font-semibold transition duration-200 flex-shrink-0 text-sm';
                                    if ($key == $activeStatus) {
                    ...
                    <div class="space-y-6">
                        ...
                        @forelse ($transactions as $transaction)
                            @php
                                ...
                                $totalItems = $transaction->items->sum('line_quantity');
                            ...
                            <div class="p-5 border border-gray-200 rounded-xl shadow-lg hover:shadow-2xl transition duration-300 bg-white">
                                ...
                                <div class="flex justify-between items-center pb-3 mb-4 border-b border-gray-100">
                                    <div>
                                        ...
                                        <p class="font-bold text-base text-gray-900">{{ $transaction->order_no }}</p>
                                    </div>
                                ...
                                <div class="flex items-start space-x-4 mb-5">
                                    @if($firstItem)
                                        <div class="w-16 h-16 bg-white rounded-lg overflow-hidden flex-shrink-0 border border-gray-300 shadow-inner">
                                ...
                                <div class="flex justify-between items-center pt-4 border-t border-gray-200">
                                    <div class="text-left">
                                        ...
                                        <p class="text-xl text-red-600 font-extrabold tracking-tight">Rp{{ number_format($transaction->total_amount, 0, ',', '.') }}</p>
                                    </div>
                                    
                                    <div class="flex space-x-3">
                                        {{-- Tombol Lihat Detail Selalu Ada --}}
                                        <a href="{{ $detailRoute }}" 
                                           class="py-2 px-4 rounded-xl text-primary font-bold text-sm border border-primary hover:bg-primary/10 transition duration-150">
                                            <i class="fas fa-search-plus mr-1"></i> Detail
                                        </a>

                                        {{-- Tombol Aksi Utama --}}
                                        <a href="{{ $currentStatus['action_url'] ?? $detailRoute }}" 
                                           class="py-2 px-5 rounded-xl text-white font-bold text-sm transition duration-150 shadow-md {{ $currentStatus['action_class'] }}">
                        ...
                        @empty
                            ...
                            <div class="text-center py-12 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-300">
                                <i class="fas fa-box-open text-7xl text-gray-400 mb-4"></i>
                                <p class="text-2xl font-semibold text-gray-700 mb-2">Riwayat Kosong</p>
                                <p class="text-gray-500 max-w-sm mx-auto">Anda belum memiliki pesanan pada filter **{{ $statusFilters[$activeStatus]['label'] ?? 'ini' }}**. Mari mulai berbelanja!</p>
                                <a href="{{ url('/') }}" class="mt-5 inline-block py-2 px-6 bg-primary text-white font-bold rounded-lg hover:bg-primary-dark transition shadow-lg">
