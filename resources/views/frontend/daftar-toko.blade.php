...
<style>
    ...
    /* Memastikan container map punya dimensi yang jelas */
    #storeMap {
        height: 500px;
        width: 100%;
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const storeIcon = L.icon({
            ...
            iconAnchor: [19, 38],
            popupAnchor: [0, -38],
            background: 'white p-2 rounded-full'
        });
        ...
        // 5. Otomatis Zoom Fit ke semua Marker yang ada
        if (branchMarkers.length > 0) {
            const group = new L.featureGroup(branchMarkers);
            map.fitBounds(group.getBounds().pad(0.1));
        }

        // Trick agar map render sempurna di dalam container dynamic
        setTimeout(() => {
            map.invalidateSize();
        }, 500);
    });
